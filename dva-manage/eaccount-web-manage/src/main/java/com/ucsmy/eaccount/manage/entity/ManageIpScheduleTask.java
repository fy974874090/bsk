...
public class ManageIpScheduleTask extends BaseNode {
     ...
     * 任务名称编码
     */
    @NotEmpty(message = "请输入任务码")//不能为null或者长度为0的(String Collection Map的isEmpty()方法)
    @Length(max = 100, message = "任务码长度不能超过100")
    @Pattern(regexp = "^(?!_)(?!.*?_$)[a-zA-Z0-9_]+$", message = "任务码为字母、数字、下划线组合，且不能以下划线开头或结尾")//给正则表达式一个匹配模式
